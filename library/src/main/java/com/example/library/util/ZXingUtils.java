...
/**
 * 类名：CreatQRCodeImg.class
 * 描述：二维码生成
 * Created by 刘帅 on 2016/9/14.
 */
public class ZXingUtils {
    private static ZXingUtils xx;
     ...
     * @return 生成的二维码
     */
    public Bitmap createQRImage_bitmap(String content, int widthPix, int heightPix, Bitmap logoBm, String filePath) {
        try {
            if (content == null || "".equals(content)) {
                return null;
            }
            ...
            //必须使用compress方法将bitmap保存到文件中再进行读取。直接返回的bitmap是没有任何压缩的，内存消耗巨大！
            if (bitmap != null)
                bitmap.compress(Bitmap.CompressFormat.JPEG, 70, new FileOutputStream(filePath));
